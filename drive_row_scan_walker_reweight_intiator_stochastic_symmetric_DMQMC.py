        ...
        if deterministic:
            ...
            for report in range(reports):
                for cycle in range(cycles):
                    ...
                    pre_nw = np.abs(d).sum()
                    trace = np.abs(d.trace())
                    d *= (Nattempts/trace)
                    d = fn.stochastic_round(d, threshold=cutoff)
                    post_nw = np.abs(d).sum()
                    delta_nw = post_nw - pre_nw
                #df = fn.write_report(iteration, tau, shift, d, Heval, df=df, stdout=verbose)
                df = fn.write_report(iteration, tau, delta_nw, d, Heval, df=df, stdout=verbose)
                if variable_shift:
                    #nw = df['Nw'][-1]
                    #nw_old = df['Nw'][-2]
