...
class TestReportFunctionsNegativeMatrix():

    @fixture(autouse=True)
    def _setup(self):
        self._matrix = np.arange(-10,15).reshape((5,5))
        self._hamiltonian = np.flip(self._matrix)

    def test_total_particles(self):
        assert np.isclose(total_particles(self._matrix), 160)

    def test_trace(self):
        assert np.isclose(trace(self._matrix), 10)
    ...
    def test_von_neumann_numerator(self):
        with raises(FloatingPointError):
            von_neumann_numerator(self._matrix)

    def test_von_neumann_expectation(self):
        with raises(FloatingPointError):
            von_neumann_expectation(self._matrix)
